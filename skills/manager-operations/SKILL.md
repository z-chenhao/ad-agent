...
description: "Triaging authorized advertiser accounts and preparing independent advertiser-scoped changes without collapsing currency, attribution, identity, or approval boundaries."
---

# Manager operations

Use this guide only when the application is in manager scope. Manager scope means
...
For a named account or direct edit, use its bound reads without a full manager scan.
For a manager-wide review, use the following evidence checks as needed:

1. Call `list_advertisers` to establish the authorized set and account metadata.
2. Call `get_manager_performance` for one explicit, comparable date window.
3. For non-trivial ranking or multi-account synthesis, call
   `run_manager_analysis` with that report ID. The isolated delegate receives
...
Include the elements relevant to the decision, without repeating rendered previews:

- manager and reporting window;
...
heterogeneous manager as one ROAS.
