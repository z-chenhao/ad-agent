...
func (s Changes) StageOperation(ctx context.Context, session store.Session, request ads.OperationRequest, reason string) (ads.Change, error) {
	if err := s.refreshPolicy(ctx); err != nil {
		return ads.Change{}, err
	}
	if s.Planner == nil {
		return ads.Change{}, errors.New("operations_unavailable")
	}
	if err := request.Validate(); err != nil {
		return ads.Change{}, errors.New("invalid_operation_request")
	}
	...
	if err := s.validateOperationPolicy(ctx, request); err != nil {
		return ads.Change{}, err
	}
	plan, err := s.Planner.PrepareOperation(ctx, request)
	if err != nil {
		return ads.Change{}, err
	}
	if len(plan.Lines) == 0 || plan.Request.Kind != request.Kind {
		return ads.Change{}, errors.New("invalid_operation_plan")
	}
	now := time.Now().UTC()
	c := ads.Change{
		ID: store.ID("change"), SessionID: session.ID, Source: a.Source,
		Kind: ads.OperationChange, Operation: &plan, State: ads.Staged,
		Reason: reason, Currency: a.Currency, SpendIncreasing: plan.SpendIncreasing,
...
func (s Changes) Apply(ctx context.Context, sessionID, id, operator string) (ads.Change, error) {
	...
	if c.Kind == ads.CreateChange && s.Creator == nil || c.Kind == ads.OperationChange && s.Operator == nil || c.Kind != ads.CreateChange && c.Kind != ads.OperationChange && s.Writer == nil {
	...
	} else if c.Kind == ads.OperationChange {
		s.applyOperation(applyCtx, &c)
...
func (s Changes) applyOperation(ctx context.Context, change *ads.Change) {
	if change.Operation == nil || s.Operator == nil {
		change.State = ads.Failed
		change.Note = "invalid_operation_payload"
		return
	}
	prepared, err := s.Operator.PrepareOperation(ctx, change.Operation.Request)
	if err != nil {
		change.State = ads.Failed
		change.Note = "operation_revalidation_failed; request not sent"
		return
	}
	if prepared.Version() != change.Operation.Version() {
		change.State = ads.Expired
		change.Note = "operation_dependencies_changed"
		return
	}
	if err := s.validateOperationPolicy(ctx, prepared.Request); err != nil {
		change.State = ads.Failed
		change.Note = err.Error() + "; request not sent"
		return
	}
	outcome := s.Operator.ApplyOperation(ctx, prepared)
	change.OperationOutcome = &outcome
	switch outcome.State {
	case "not_sent", "rejected":
		change.State = ads.Failed
	case "acknowledged":
		matched, reconcileErr := s.Operator.ReconcileOperation(ctx, prepared, outcome)
		if reconcileErr == nil && matched {
			change.State = ads.Applied
			change.Note = "acknowledged and read-back matched"
		} else {
			change.State = ads.Indeterminate
			change.Note = "operation acknowledged; read-back not confirmed"
		}
	case "partial":
		change.State = ads.Indeterminate
		change.Note = "operation partially completed; no automatic retry"
	default:
		change.State = ads.Indeterminate
		change.Note = "remote outcome unknown; no automatic retry"
	}
}
...
func (s Changes) Reconcile(ctx context.Context, sessionID, id string) (ads.Change, error) {
	...
	if c.Kind == ads.OperationChange {
		if c.Operation == nil || c.OperationOutcome == nil || s.Operator == nil {
			return c, errors.New("change_not_reconcilable")
		}
		matched, observeErr := s.Operator.ReconcileOperation(ctx, *c.Operation, *c.OperationOutcome)
		if observeErr != nil || !matched {
			return c, observeErr
		}
		c.State = ads.Applied
		c.Note = "current resources match the requested operation; execution attribution unconfirmed"
		err = s.Store.Transition(ctx, ads.Indeterminate, c)
		return c, err
	}
