...
func TestClaudeAllowsUnboundedMainLoopRounds(t *testing.T) {
	entry, _ := filepath.Abs("testdata/claude-bridge.mjs")
	dir := t.TempDir()
	t.Setenv("ANTHROPIC_API_KEY", "test-only")
	t.Setenv("CLAUDE_FAKE_SESSION_DIR", dir)
	selection := ModelSelection{Provider: "anthropic", Model: "claude-sonnet-4-6", Reasoning: "medium", AuthMode: APIKeyAuth, API: AnthropicMessages, BaseURL: "https://api.anthropic.com", APIKeyEnv: "ANTHROPIC_API_KEY", ContextWindow: 200000, MaxOutputTokens: 16000}
	calls := 0
	result, err := (Claude{Entry: entry}).Run(context.Background(), Request{System: "system", Prompt: "read", Model: selection, SessionDir: dir, MaxRounds: 0, Tools: []Tool{{Name: "read_data", Description: "read", Parameters: json.RawMessage(`{"type":"object"}`)}}}, Hooks{Execute: func(_ context.Context, call Call) ToolResult {
		calls++
		if call.Round != 20 {
			t.Fatalf("call=%#v", call)
		}
