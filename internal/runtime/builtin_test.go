...
func TestBuiltinToolLoopCheckpointAndRestore(t *testing.T) {
	entry, err := filepath.Abs("testdata/builtin-model-bridge.mjs")
	if err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	runtime := Builtin{Entry: entry}
	...
	if err == nil || FailureCode(err) != "runtime_checkpoint_invalid" {
		t.Fatalf("checkpoint mismatch err=%v", err)
	}
}

func TestBuiltinMarksBackstopAsBudgetExhausted(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	result, err := (Builtin{Entry: entry}).Run(context.Background(), Request{
...
func TestBuiltinAllowsUnboundedMainLoopRounds(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	executed := 0
	result, err := (Builtin{Entry: entry}).Run(context.Background(), Request{
		System: "system", Prompt: "first", MaxRounds: 0,
		Tools: []Tool{{Name: "read_data", Description: "read", Parameters: json.RawMessage(`{"type":"object"}`)}},
	}, Hooks{Execute: func(context.Context, Call) ToolResult {
		executed++
		return Value("ok")
	}})
	if err != nil || result.Stop != "stop" || result.Text != "done" || executed != 1 {
		t.Fatalf("result=%#v executed=%d err=%v", result, executed, err)
	}
}

func TestBuiltinDispatchesIndependentCallsConcurrently(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	started := 0
	var mu sync.Mutex
...
func TestBuiltinCancellationStopsBridge(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	ctx, cancel := context.WithTimeout(context.Background(), 100*time.Millisecond)
	defer cancel()
	_, err := (Builtin{Entry: entry}).Run(ctx, Request{System: "system", Prompt: "hang", MaxRounds: 1}, Hooks{})
	if err == nil || ctx.Err() == nil {
		t.Fatalf("err=%v ctx=%v", err, ctx.Err())
	}
}

func TestBuiltinRejectsCheckpointOutsideRuntimeRoot(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	root := t.TempDir()
	...
	_, err := (Builtin{Entry: entry}).Run(context.Background(), Request{
		System: "system", Prompt: "first", MaxRounds: 1,
...
func TestBuiltinPublishesCompletionOnlyContentBeforeTools(t *testing.T) {
	entry, _ := filepath.Abs("testdata/builtin-model-bridge.mjs")
	var events []string
	result, err := (Builtin{Entry: entry}).Run(context.Background(), Request{
		System: "system", Prompt: "content_with_tools",
		Tools: []Tool{{Name: "read_data", Description: "read", Parameters: json.RawMessage(`{"type":"object"}`)}},
	}, Hooks{
		...
		Execute: func(context.Context, Call) ToolResult { events = append(events, "tool"); return Value("ok") },
	})
	if err != nil || result.Text != "done" || strings.Join(events, "|") != "I will inspect the account.|tool|done" {
