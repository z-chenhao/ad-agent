...
type Request struct {
	...
	// MaxRounds bounds narrowly scoped internal passes. Zero leaves the main
	// agent loop without a fixed model-round ceiling; host safety limits still apply.
	// Codex conservatively bounds tool dispatches because native model rounds are
	// not exposed. This is a private allowance, not portable token/round accounting.
