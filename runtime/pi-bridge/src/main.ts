...
async function run(req: Start) {
  ...
  const publicText = new PublicText(send);
  session.subscribe((event) => {
    if (event.type === "message_start" && event.message.role === "assistant")
      publicText.start();
    if (event.type === "message_end" && event.message.role === "assistant") {
      event.message.content.forEach((block, index) => {
        if (block.type === "text") publicText.complete(index, block.text);
      });
    }
    if (event.type === "turn_start") {
      round++;
      // At the next model step remove tools; Go independently rejects over-budget business calls.
      if (req.max_rounds > 0 && round > req.max_rounds) {
        budget = true;
        session!.setActiveToolsByName([]);
      }
      if (req.max_rounds > 0 && round > req.max_rounds + 1)
        void session!.abort();
    }
    if (
