...
export type InputFrame =
  ...
  | { type: "complete"; id: string; request: BuiltinRequest };
...
export function buildContext(
  ...
  for (let index = 0; index < request.messages.length; index++) {
    ...
    validateBuiltinMessage(message);
    ...
    if (
      ...
      canonical(normalizeNative(native, selection)) !==
        canonical(normalizeBuiltin(message.content))
    )
...
export function toBuiltinMessage(
...
function validateBuiltinMessage(message: BuiltinMessage): void {
  if (
    !message ||
...
function joinText(message: BuiltinMessage): string {
...
function normalizeBuiltin(content: BuiltinContent[]): BuiltinContent[] {
...
function validModel(model: Record<string, unknown>): boolean {
  ...
  if (model.auth_mode === "chatgpt_oauth")
    return (
      ...
      supportedModels.has(String(model.model))
    );
  return (
    model.auth_mode === "api_key" &&
    typeof model.provider === "string" &&
    /^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$/.test(model.provider) &&
    typeof model.model === "string" &&
    /^[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}$/.test(model.model) &&
    ["anthropic-messages", "openai-responses", "openai-completions"].includes(
      String(model.api),
    ) &&
    ...
    Number(model.context_window) >= 4096 &&
    Number.isInteger(model.max_output_tokens) &&
    Number(model.max_output_tokens) >= 256
  );
}
