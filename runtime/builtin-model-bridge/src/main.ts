...
import { providerFailure, providerFailureCodes } from "./errors.js";
...
async function complete(id: string, request: BuiltinRequest) {
  ...
  for await (const event of stream) {
    ...
    if (typed.type === "text_delta") {
      ...
      throw new Error(providerFailure(typed.error.errorMessage));
    }
  }
  ...
  send({
    ...
    id,
    response: {
      message: toBuiltinMessage(final, selection),
