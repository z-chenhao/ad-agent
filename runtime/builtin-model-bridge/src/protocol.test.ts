...
test("provider diagnostics never forward raw messages or request fragments", () => {
  assert.equal(
    providerFailure("Duplicate item id secret-value"),
    "provider_duplicate_item",
  );
  assert.equal(
    providerFailure("Invalid encrypted content secret-value"),
    "provider_encrypted_state_rejected",
  );
  assert.equal(
    providerFailure("No tool output found for private-call"),
    "provider_history_rejected",
  );
  assert.equal(
    providerFailure("fetch failed https://private.test?key=secret-value"),
    "provider_transport_failed",
  );
  assert.equal(
    providerFailure("arbitrary private response"),
    "provider_failed",
  );
});
...
test("replays only matching native assistant state", () => {
  ...
  const j = toBuiltinMessage(native, selection);
